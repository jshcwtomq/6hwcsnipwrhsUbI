	...
	CreateWindow = function(self, op)
		...
		function g:CreateTab(gfjd)
			...
			Tab.MouseEnter:Connect(function()
				if not Tab:GetAttribute("Selected") and Tab.BackgroundTransparency > 0 then
					b[1]().tw({
						v = Tab,
						t = 0.2,
						s = "Linear",
			...
			Tab.MouseLeave:Connect(function()
				if not Tab:GetAttribute("Selected") and Tab.BackgroundTransparency < 1 and Tab.BackgroundTransparency > 0 then
					b[1]().tw({
						v = Tab,
						t = 0.2,
						s = "Linear",
			...
			function Func:CreateToggle(khgkgh)
				...
				local par = b[1]().background(self._Scroll or Scroll, khgkgh.Title, khgkgh.Desc, false, op)
			...
			function Func:CreateLabel(khgkgh)
				...
				local par = b[1]().background(self._Scroll or Scroll, khgkgh.Title, khgkgh.Desc, true, op)
			...
			function Func:CreateButton(khgkgh)
				...
				local par, Callback = b[1]().background(self._Scroll or Scroll, khgkgh.Title, khgkgh.Desc, false, op), khgkgh.Callback or function() end
				par.TextDesc.TextLabel.TextTransparency = 0
			...
			function Func:CreateTextbox(khgkgh)
				...
				local par, Callback, Placeholder, Value, ClearTextOnFocus = b[1]().background(self._Scroll or Scroll, khgkgh.Title, khgkgh.Desc, false, op), khgkgh.Callback or function() end, khgkgh.Placeholder or "Paste Your Text", khgkgh.Value or "", khgkgh.ClearTextOnFocus or false
				par.TextDesc.TextLabel.TextTransparency = 0
			...
			function Func:CreateSlider(khgkgh)
				...
				local par, Callback, Value, Min, Max, DecimalPlaces = 
					b[1]().background(self._Scroll or Scroll, khgkgh.Title, khgkgh.Desc, true, op), 
			...
			function Func:CreateSection(khgkgh)
				...
				-- Folder support is additive: all existing Func controls are
				-- reused through the folder's private scroll container.
				function Func:CreateFolder(khgkgh)
					assert(khgkgh.Title, "Folder - Missing Title")

					local folderRoot = f("Frame", {
						Parent = self._Scroll or Scroll,
						Name = "Folder_" .. tostring(khgkgh.Title),
						BackgroundTransparency = 1,
						Size = UDim2.new(1, 0, 0, 36),
						BorderSizePixel = 0
					})

					local folderHeader = f("TextButton", {
						Parent = folderRoot,
						BackgroundColor3 = a.Theme[op.Theme or 'Quizzy']['Background Function'],
						BackgroundTransparency = a.Theme[op.Theme or 'Quizzy']['Background Function Transparency'],
						BorderSizePixel = 0,
						Size = UDim2.new(1, 0, 0, 34),
					...
					local folderScroll = f("ScrollingFrame", {
						Parent = folderRoot,
						Active = true,
						BackgroundTransparency = 1,
						BorderSizePixel = 0,
					...
					local isOpen = khgkgh.Open ~= false
					local function updateFolderSize()
						folderScroll.CanvasSize = UDim2.new(0, 0, 0, folderScroll.UIListLayout.AbsoluteContentSize.Y + 8)
						if isOpen then
							folderRoot.Size = UDim2.new(1, 0, 0, 43 + folderScroll.UIListLayout.AbsoluteContentSize.Y)
						else
							folderRoot.Size = UDim2.new(1, 0, 0, 36)
						end
					end
					...
					folderHeader.MouseEnter:Connect(function()
						b[1]().tw({
							...
							s = "Linear",
							d = "Out",
							g = {BackgroundTransparency = a.Theme[op.Theme or 'Quizzy']['Background Function Transparency Moved']}
						}):Play()
					end)
					...
					folderHeader.MouseLeave:Connect(function()
						b[1]().tw({
							...
							s = "Linear",
							d = "Out",
							g = {BackgroundTransparency = a.Theme[op.Theme or 'Quizzy']['Background Function Transparency']}
						}):Play()
					end)

					folderHeader.MouseButton1Click:Connect(function()
						setFolderOpen(not isOpen)
					end)

					folderScroll.UIListLayout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(updateFolderSize)
					setFolderOpen(isOpen)

					local FolderFunc = setmetatable({
						_Scroll = folderScroll,
						_FolderRoot = folderRoot,
					}, {__index = Func})

					function FolderFunc:SetOpen(value)
						setFolderOpen(value)
					end

					function FolderFunc:Toggle()
						setFolderOpen(not isOpen)
					end

					function FolderFunc:SetVisible(value)
						folderRoot.Visible = value
					end

					function FolderFunc:SetTitle(value)
						folderHeader.FolderTitle.Text = value
					end
...
-- Additive export: existing scripts can still use global b, while
-- loadstring(... )() can now receive the same library table directly.
return b
