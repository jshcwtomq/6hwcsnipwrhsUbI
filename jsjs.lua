	...
	[1] = function()
		...
		function x.tw(info)
			return game:GetService"TweenService":Create(info.v,TweenInfo.new(info.t,Enum.EasingStyle[info.s],Enum.EasingDirection[info.d]),info.g)
				...
				-- Keep one tween per property owner.  The old UI stacked tweens on
				-- every hover/refresh, which was the main source of the input lag.
				function x.stop(o)
					for _, t in ipairs(o:GetChildren()) do
						if t:IsA("Tween") then pcall(function() t:Cancel() end) end
					end
				end
		function x.lak(o)
			local a, b, c, d
	...
	CreateWindow = function(self, op)
		...
		function g:CreateTab(gfjd)
			...
			local function selectTab()
				local currentSelection = (fo:GetAttribute("SelectionSerial") or 0) + 1
				fo:SetAttribute("SelectionSerial", currentSelection)
				...
				for i, v in pairs(fo:GetChildren()) do
					if v:IsA("Frame") and v.Name == "Page" and v ~= Page then
						...
						if v:FindFirstChild("CanvasGroup") then
							v.CanvasGroup.GroupTransparency = 1
						end
					end
				end
				...
				-- Ignore an older click's completion if the user changed tabs quickly.
				if currentSelection ~= fo:GetAttribute("SelectionSerial") then return end
				-- Show page with fade in
				Page.Visible = true
				Page.CanvasGroup.GroupTransparency = 1
				b[1]().tw({
					v = Page.CanvasGroup,
					t = 0.25,
					s = "Linear",
			...
			delay(0.1, function()
				if not fo:GetAttribute("HasSelectedTab") then
					selectTab()
					fo:SetAttribute("HasSelectedTab", true)
				end
			end)
			...
			function Func:CreateToggle(khgkgh)
				...
				local dropdownselect = f("Frame", {
					...
					f("TextBox", {
						...
						TextColor3 = a.Theme[op.Theme or 'Quizzy']['Text Color'],
						PlaceholderColor3 = a.Theme[op.Theme or 'Quizzy']['Text Color'],
				...
				local isopen = false
				local click = b[1]().click(par)
				local SearchBox = dropdownselect.SearchBox
				local function filterItems()
					...
					for _, child in ipairs(dropdownselect.ItemList:GetChildren()) do
						if child:IsA("Frame") and child:FindFirstChild("TextLabel") then
							child.Visible = query == "" or string.find(string.lower(child.TextLabel.Text), query, 1, true) ~= nil
						end
					end
					dropdownselect.ItemList.CanvasPosition = Vector2.new(0, 0)
				...
				local function opendropdown()
					...
					local desiredHeight = math.min(200, dropdownselect.ItemList.UIListLayout.AbsoluteContentSize.Y + 40)
					if desiredHeight < 120 then
						b[1]().tw({
							v = dropdownselect,
							t = 0.15,
							s = "Exponential",
							d = "InOut",
							g = {Size = UDim2.new(0, 150,0, desiredHeight)}
						}):Play()
						b[1]().tw({
						...
						b[1]().tw({
							...
							s = "Exponential",
							d = "InOut",
							g = {Size = UDim2.new(0, 150,0, desiredHeight), Position = UDim2.new(0, targetX, 0, targetY)}
						}):Play()
					end
				end
				...
				function itemslist:SetList(newList, autoSelectValue)
					-- Refreshes must not clear a player that is still in the server.
					-- Preserve the current value, then restore only values present
					-- in the refreshed list (an actually gone player is removed).
					local previousValue = Value
					local previousMulti = {}
					if Multi then
						for key in pairs(selectedValues) do
							previousMulti[key] = true
						end
					end
					self:Clear()
					Value = ""
					...
					elseif Multi then
						local keep = {}
						for value in pairs(previousMulti) do
							if available[value] then table.insert(keep, value) end
						end
						if #keep > 0 then
							task.defer(function() self:SetValue(keep) end)
						end
					elseif previousValue ~= nil and previousValue ~= "" and available[previousValue] then
						task.defer(function() self:SetValue(previousValue) end)
					end
					dropdownselect.ItemList.CanvasSize = UDim2.new(0,0,0, dropdownselect.ItemList.UIListLayout.AbsoluteContentSize.Y + 5)
					dps()
				end
